  ...
  try {
    ...
    const response = await fetch(API_URL, {
      ...
      headers: {
        ...
        "HTTP-Referer": window.location.origin, // change for production
        "User-Agent": "symptom-checker-app", // Optional. Site title for rankings on openrouter.ai.
    ...
    .map(
      (symptom: Symptom) =>
        `- ${symptom.description} (Severity: ${
          symptom.severity
        }/10, Duration: ${symptom.durationHours} hours, Location: ${
  ...
  const medicalHistoryText =
    patientData.medicalHistory && patientData.medicalHistory.length > 0
      ? patientData.medicalHistory
          .map(
            (h) =>
              `- ${h.condition} (${h.diagnosisDate || "Unknown"}) | Status: ${
                h.status || "Unknown"
              }, Severity: ${h.severity || "Unknown"}${
                h.treatment ? ", Treatment: " + h.treatment : ""
              }${h.notes ? ", Notes: " + h.notes : ""}${
                h.relatedSymptoms && h.relatedSymptoms.length > 0
                  ? ", Related Symptoms: " + h.relatedSymptoms.join(", ")
                  : ""
              }`
          )
          .join("\n")
  ...
  const medicationsText =
    patientData.medications && patientData.medications.length > 0
      ? patientData.medications
          .map(
            (m) =>
              `- ${m.name} ${m.dosage} (${m.frequency}) | Times: ${
                m.times && m.times.length > 0 ? m.times.join(", ") : "N/A"
              }, Start: ${m.startDate}${
                m.endDate ? ", End: " + m.endDate : ""
              }${m.notes ? ", Notes: " + m.notes : ""}, Active: ${
                m.isActive ? "Yes" : "No"
              }, Reminder: ${m.reminderEnabled ? "On" : "Off"}${
                m.lastTaken ? ", Last Taken: " + m.lastTaken : ""
              }`
          )
          .join("\n")
